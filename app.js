...
async function handleLogin(id, pw) {
    ...
    try {
        ...
        try {
            ...
            // HTML 에러가 왔다면 구글 스크립트 에러일 가능성 높음
...
async function handleSignup() {
    ...
    if (id.length !== 4) return alert("학번은 4자리 (예: 1213)");
...
window.submitGalleryPost = async () => {
    ...
    reader.onload = async (e) => {
        ...
        try {
            ...
            let json;
            try { json = JSON.parse(text); }
            catch (err) { throw new Error("서버 응답 오류 (HTML): " + text.substring(0, 100)); }
            ...
            if (json.success) {
                ...
                // ★ 서버가 보낸 구체적인 에러 메시지 출력 (구글 드라이브 실패 등)
                alert("업로드 실패:\n" + (json.error || "알 수 없는 오류"));
            }
...
function renderGalleryChunk() {
    ...
    nextBatch.forEach(item => {
        // 이미지 소스 (Content가 우선, URL 형태면 그대로 씀)
