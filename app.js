        ...
        loginForm.onsubmit = async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    alert("로그인 실패: " + (data.error || "비밀번호나 승인 여부를 확인하세요."));
                    btn.disabled = false; btn.textContent = "로그인";
                }
            } catch (err) { alert("서버 응답 오류 (로그인)"); btn.disabled = false; }
        };
    }
    ...
    if (signupForm) {
        signupForm.onsubmit = async (e) => {
            ...
            try {
                ...
                if (!text || text.trim() === "") {
                    throw new Error("서버에서 아무런 대답이 없습니다. (GAS 배포 상태를 확인하세요)");
                }
                ...
                try {
                    ...
                    throw new Error("서버가 올바른 형식이 아닌 대답을 보냈습니다:\n" + text.substring(0, 50));
                }

                if (data.success) {
                    alert("✅ 가입 신청 완료! 담임 선생님의 승인을 기다리세요.");
                    location.reload();
                } else {
                    alert("⚠️ 가입 신청 실패: " + (data.error || "상세 사유 없음"));
