...
function setupByteCounter(inputId, counterId, maxBytes, submitBtnId) {
    ...
    const update = () => {
        ...
        if (len > maxBytes) { counter.classList.add('over'); input.style.borderColor = "#ff4757"; if (btn) btn.disabled = true; }
        else { counter.classList.remove('over'); input.style.borderColor = "#ddd"; if (btn) btn.disabled = false; }
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ★ 네비게이션 클릭 이벤트 복구 (여기가 문제였습니다!)
    document.querySelectorAll('.nav-links li').forEach(li => {
        li.addEventListener('click', () => {
            const id = li.id;
            if (id === 'home-link') navigateTo('home');
            else if (id === 'community-link') navigateTo('community');
            else if (id === 'gallery-link') navigateTo('gallery');
            else if (id === 'notice-link') navigateTo('notice');
            else if (id === 'lunch-link') getEl('lunch-modal').style.display = 'flex';
            else if (id === 'login-link') {
                if (currentUser) {
    ...
    if (fileInput) {
        fileInput.addEventListener('change', () => {
            ...
            if (file) {
                if (!file.name.toLowerCase().match(/\.(jpg|jpeg|png)$/i)) { alert("이미지 파일만 가능합니다! (png, jpg, jpeg)"); fileInput.value = ''; return; }
                if (file.size > 5 * 1024 * 1024) { alert("파일 크기가 5MB를 넘습니다!"); fileInput.value = ''; return; }
...
async function checkDot(key, type, todayStr) {
    ...
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            if (isToday(data[0].date)) getEl(`dot-${key}`).style.display = 'block';
        }
    } catch (e) { }
}
...
function closeModal(modalId) { getEl(modalId).style.display = 'none'; }
...
async function submitPostGeneric(prefix, type) {
    ...
    try {
        ...
        loadBoard(type === 'Post' ? 'community' : 'notice');
...
function renderBoard(boardName, page) {
    ...
    if (allPosts.length === 0) { list.innerHTML = '<div style="text-align:center; padding:20px;">글이 없습니다.</div>'; return; }
    ...
    pagePosts.forEach(post => {
        ...
        `;
        if (!isAuthorized) {
            contentHtml += `<div class="stamp-overlay"><div class="stamp-text-1">가입/승인</div><div class="stamp-text-2">후</div><div class="stamp-text-3">열람!</div></div>`;
    ...
    if (totalPages > 1) {
        if (page > 1) {
            const btn = document.createElement('button'); btn.className = "page-btn"; btn.innerText = "<";
            btn.onclick = () => renderBoard(boardName, page - 1); pagination.appendChild(btn);
        }
        for (let i = 1; i <= totalPages; i++) {
            const btn = document.createElement('button'); btn.className = `page-btn ${i === page ? 'active' : ''}`; btn.innerText = i;
            btn.onclick = () => renderBoard(boardName, i); pagination.appendChild(btn);
        }
        if (page < totalPages) {
            const btn = document.createElement('button'); btn.className = "page-btn"; btn.innerText = ">";
            btn.onclick = () => renderBoard(boardName, page + 1); pagination.appendChild(btn);
...
function openDetail(post) {
    ...
    const dateStr = formatDateTwoLines(post.date).replace('<br>', ' ');
    ...
    reader.onload = async (e) => {
        ...
        try {
            const res = await fetch(CONFIG.API_URL, {
                method: 'POST',
                body: JSON.stringify({ Title: title, Author: currentUser, Type: 'Gallery', Image: base64Data, MimeType: file.type })
            });
            const text = await res.text();
        ...
        finally { if (btn) { btn.innerText = "올리기"; btn.disabled = false; } }
    };
