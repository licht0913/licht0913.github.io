...
function setupByteCounter(inputId, counterId, maxBytes, submitBtnId) {
    ...
    const update = () => {
        ...
        if (len > maxBytes) {
            ...
            input.style.borderColor = "#ff4757"; // 빨강
            input.style.backgroundColor = "#fff0f0"; // 연한 빨강 배경
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 나머지 메뉴
    if (getEl('home-link')) getEl('home-link').onclick = () => navigateTo('home'); // 홈 링크가 없다면 로고 클릭으로 대체됨
    // 로고 클릭
...
async function checkDot(key, type, todayStr) {
    ...
    // if (lastRead === todayStr) return; // (테스트를 위해 잠시 주석 처리 가능하나 원복)
...
function updateUI_LoggedIn(name, role) {
    ...
    link.innerHTML = `👤 ${name}`;
    // 로그인 상태면 클릭 이벤트가 로그아웃으로 동작하도록 설정 (DOM 로드 시점과는 별개로 업데이트 필요)
    link.onclick = () => {
        if (confirm("로그아웃 하시겠습니까?")) {
            localStorage.removeItem('user_name');
            localStorage.removeItem('user_role');
