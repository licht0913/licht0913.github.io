    ...
    if (loginLink) {
        loginLink.onclick = (e) => {
            if (currentUser) {
                ...
                // [수정] 로그인 버튼 클릭 시 확실하게 Auth 페이지와 Login View를 켭니다.
                navigateTo('auth');
...
function navigateTo(pageId) {
    if (pageId === 'auth') {
        window.toggleAuthMode('login'); // 기본값
        return;
    }
...
function renderBoard(boardName, page) {
    ...
    pagePosts.forEach(post => {
        ...
        if (!isAuthorized) {
            // [수정] 비인증 상태일 때: 도장을 포함하여 렌더링 (호버 여부 상관없이 보임)
            contentHtml = `
                <div class="blur-it">
...
// Infinite Scroll Gallery logic preserved
...
function renderGalleryChunk() {
    ...
    nextBatch.forEach(item => {
        ...
        if (!isAuthorized) {
            ...
            contentHtml = `<img src="${imgSrc}" loading="lazy">`; // 갤러리는 깔끔하게 이미지만
