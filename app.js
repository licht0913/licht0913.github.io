...
function updateAuthUI() {
    ...
    if (currentUser) {
        ...
        // Show write buttons
        getEl('btn-write').style.display = 'flex';
        getEl('btn-gallery-write').style.display = 'flex';
        getEl('btn-notice-write').style.display = (currentUser.role === 'Teacher') ? 'flex' : 'none';

        // Unblur (pages handled in navigateTo, but let's ensure overall targets)
        ...
        // Hide write buttons
        getEl('btn-write').style.display = 'none';
        getEl('btn-gallery-write').style.display = 'none';
        getEl('btn-notice-write').style.display = 'none';
...
function renderList(list, category, container) {
    ...
    list.forEach(item => {
        ...
        if (category === 'Community') {
            ...
            div.innerHTML = `<img src="${item.content.startsWith('data') ? item.content : 'https://picsum.photos/300'}" style="width:100%" onclick="window.openImageModal('${item.content}')">`;
...
function initWriteForms() {
    getEl('btn-write').onclick = () => getEl('write-modal').classList.add('show');
    getEl('btn-gallery-write').onclick = () => getEl('gallery-modal').classList.add('show');
    getEl('btn-notice-write').onclick = () => getEl('notice-modal').classList.add('show');
}
