...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (savedUser) {
        ...
        isAuthorized = false;
        // 비로그인 상태면 홈 화면 보여주되, 로그인 버튼은 활성화되어야 함
    }
    ...
    if (loginLink) {
        loginLink.onclick = (e) => {
            if (currentUser) {
                ...
                // 여기가 문제였을 수 있음: 확실하게 페이지 전환 + 뷰 전환
                navigateTo('auth');
                toggleAuthMode('login'); // 로그인 폼 보여주기 강제 실행
            }
        };
    }

    // 나머지 메뉴 이벤트
    if (getEl('home-link')) getEl('home-link').onclick = () => navigateTo('home'); // 홈 메뉴가 없을 수 있으므로 예외처리
...
// Auth Toggle (전역 함수로 등록)
window.toggleAuthMode = (mode) => {
    // 1. 페이지를 auth로 이동 (혹시 다른 페이지에 있을 경우를 대비)
    showPage('auth');
    ...
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            if (isToday(data[0].date)) {
                if (lastRead !== todayStr) getEl(`dot-${key}`).style.display = 'block';
            }
        }
    } catch (e) { }
}
...
function navigateTo(pageId) {
    if (pageId === 'auth') {
        // Auth 페이지로 갈 때는 showPage만 부르면 됨 (toggleAuthMode는 클릭 이벤트에서 처리하거나 여기서 기본값 설정)
        // 기본적으로 로그인 화면을 보여주도록 설정 가능하나, toggleAuthMode에서 처리
        showPage('auth');
        return;
    }
    if (pageId === 'home') { showPage(pageId); return; }
...
function showPage(pageId) {
    ...
    document.querySelectorAll('.page').forEach(p => {
        ...
        p.style.display = 'none'; // 안전장치: 확실하게 숨김
    });
    ...
    if (targetPage) {
        ...
        targetPage.style.display = 'block'; // 안전장치: 확실하게 보임
    }
}
...
async function handleSignup() {
    ...
    try {
        ...
        if (json.success) {
            alert("가입 신청 완료!\n선생님의 승인을 기다려주세요.");
            toggleAuthMode('login'); // 성공 후 로그인 화면으로 전환
        } else {
...
function updateUI_LoggedIn(name, role) {
    ...
    // 로그아웃 이벤트는 DOMContentLoaded에서 이미 연결됨 (조건부처리)
    // 하지만 안전을 위해 여기서 갱신 가능
    link.onclick = () => {
        if (confirm("로그아웃 하시겠습니까?")) {
            localStorage.removeItem('user_name');
            localStorage.removeItem('user_role');
