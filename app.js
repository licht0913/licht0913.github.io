...
function formatDateSingleLine(isoString) {
    if (!isoString) return "";
    const d = new Date(isoString);
    const yy = d.getFullYear().toString().slice(2);
    const m = d.getMonth() + 1;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (getEl('home-link')) getEl('home-link').onclick = () => navigateTo('home');
...
async function checkDot(key, type, todayStr) {
    try {
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            if (isToday(data[0].date) && lastRead !== todayStr) {
...
function navigateTo(pageId) {
    if (pageId === 'auth') { window.toggleAuthMode('login'); return; }
...
function renderGalleryChunk() {
    ...
    nextBatch.forEach(item => {
        ...
        if (!isAuthorized) {
            // [수정] class="gallery-item locked" 부여하여 CSS 필터 고정
            div.className = "gallery-item locked";
            contentHtml = `
                    <img src="${imgSrc}">
                <div class="stamp-overlay" style="width:120px; height:120px; font-size:1.1rem;">
                    <div class="stamp-text-1">가입/승인</div>
                    <div class="stamp-text-2">후</div>
                    <div class="stamp-text-3">열람!</div>
                </div>
            ...
            const dateStr = formatDateSingleLine(item.date);
            contentHtml = `
                <img src="${imgSrc}" loading="lazy">
                <div class="gallery-overlay">
                    <div class="gal-title">${item.title}</div>
                    <div class="gal-meta">${item.author} | ${dateStr}</div>
                </div>
            `;
